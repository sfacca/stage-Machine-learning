struct MvSkewTDist <: ContinuousMultivariateDistribution
    ξ::Vector{Float64}
    Ω::PDMat
    α::Vector{Float64}
    df::Float64
...
function _rand!(dist::MvSkewTDist, x::AbstractVector{T}) where {T<:Real}
...
function _logpdf!(r::AbstractArray, dist::MvSkewTDist, X::DenseMatrix{T}) where {T<:Real}
    k = length(dist)
...
function _logpdf(dist::MvSkewTDist, x::AbstractVector{T}) where {T<:Real}
    k = length(dist)
